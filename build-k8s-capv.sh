...
cp Containerfile.k8s-capv "$DEST/Containerfile"
cp -r kube "$DEST"
cp -r repos "$DEST"
cp -r dracut "$DEST"
cp -r ign-converter "$DEST"

cd build

cosas build --version=stable
cosas buildextend-qemu
cosas buildextend-vmware
img=$(sudo ls builds/latest/x86_64/ | sudo grep ova)
mv builds/latest/x86_64/$img $(pwd)/../fcos-k8s-vmware.ova
