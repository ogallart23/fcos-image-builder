...
func main() {
	...
	if err := json.Unmarshal(dataIn, &generic); err != nil {
		fail("failed to unmarshal JSON for TLS removal: %v", err)
	}
	if ignition, ok := generic["ignition"].(map[string]interface{}); ok {
		if security, ok := ignition["security"].(map[string]interface{}); ok {
			delete(security, "tls")
		}
	}
	// Tornar a convertir a bytes per passar al parser
	dataIn, err = json.Marshal(generic)
	if err != nil {
		fail("failed to re-marshal JSON after TLS removal: %v", err)
	}
