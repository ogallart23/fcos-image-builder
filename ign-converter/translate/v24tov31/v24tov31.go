...
func Translate(cfg old.Config, fsMap map[string]string) (types.Config, error) {
	...
	cfg, err = RemoveDuplicateFilesUnitsUsers(cfg)
	if err != nil {
		...
		if _, isDup := unitNameMap[unitName]; isDup {
			...
			for j := range outUnits {
				if outUnits[j].Name == unitName {
					...
					// 🔹 NOU: hereta contents si el guanyador està buit
					if outUnits[j].Contents == "" && units[i].Contents != "" {
						outUnits[j].Contents = units[i].Contents
					}
